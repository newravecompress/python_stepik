...
def is_parent(exception):
    global tree, exception_list
    result = ''
...
for i in range(num):
    ...
    if ':' in str:
        ...
        tree[exceptions[0]] = [val for val in exceptions[1].split()]
    else:
        tree[str] = ['object']
...
for j in range(num2):
    ...
    if new_exception in exception_list:
        continue

    for parent in tree[new_exception]:
        if is_parent(parent):
            print(new_exception)
            break

    exception_list.append(new_exception)
