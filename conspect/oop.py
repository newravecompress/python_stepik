# Объект - своего рода коробка для вещи на складе. Любая вещь сначала кладется в коробку, а потом помещается
# на склад, то есть в память. Но объект хранит в себе не только сами данные, но и отношения между ними.
# Пример - список
a = 1
b = 2
c = [a, b]
# У объектов есть 3 обязательх характеристики: идентификатор, тип, значение
# Идентификатор у объекта не меняется
x = [1, 2, 3]
print(id(x))
...
y is x          # True
y is [1, 2, 3]  # False
# Переменная - всегда ссылка на объект
...
# В отличие от функций, тело класса исполняется в момент его определения
# Для класса определяется свой неймспейс
